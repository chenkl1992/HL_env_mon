...
uint8_t DM9000_Init(void)
{
	...
	*/
	...
	uint32_t sn = *(uint32_t*)(0x1FFFF7E8);
        uint32_t sn1 = *(uint32_t*)(0x1FFFF7EC);
        uint32_t sn2 = *(uint32_t*)(0x1FFFF7F0);
        uint8_t crcdata[12] = {0};
        uint16_t crc = 0;
        memcpy(crcdata, &sn, 4);
        memcpy(crcdata+4, &sn1, 4);
        memcpy(crcdata+8, &sn2, 4);
        crc = crc16_modbus(crcdata, 16);
	dm9000cfg.mac_addr[0] = 2;
	dm9000cfg.mac_addr[1] = 0;
	dm9000cfg.mac_addr[2] = (sn>>8)&0xFF;
	dm9000cfg.mac_addr[3] = sn&0xFF;
	dm9000cfg.mac_addr[4] = crc&0xFF;
	dm9000cfg.mac_addr[5] = (crc >> 8)&0xFF;
	//初始化组播地址
	...
	loginf("DM9000 ID:%x\r\n",temp);
        logerr("DM9000 Mac:%02x %02x %02x %02x %02x %02x\r\n", dm9000cfg.mac_addr[0], dm9000cfg.mac_addr[1], dm9000cfg.mac_addr[2],dm9000cfg.mac_addr[3],dm9000cfg.mac_addr[4],dm9000cfg.mac_addr[5]);
