...
static uint32_t _get_jedec_id(void)
{
	...
	uint8_t  buf[] = {0x9F, DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE};
        //uint8_t  buf[] = {0x90, 0x00, 0x00, 0x00};
...
{	
	...
	if (id != SFLASH_CHIP)
	{
		printf("Flash Get JEDEC_ID Failed. ID = 0x%.8X.\r\n", id);
		return 0;
	}
	printf("Flash Get JEDEC Success.\r\n");
...
uint8_t sFlash_EraseChip(void)
{
	...
	_UNLOCK;
	if(!is_success)
		printf("sFlash_EraseChip Failed.\r\n");
...
uint8_t sFlash_EraseSector(uint32_t addr)
{
	if ((addr & (SIZE_4KB - 1)) ||
		(addr >= SFLASH_SIZE))
	{
		printf("sFlash_EraseSector Args Error. addr = 0x%.8X.\r\n", addr);
	...
	_UNLOCK;
	if(!is_success)
		printf("sFlash_EraseSector Failed.\r\n");
	return is_success ? SFLASH_ERR_NONE : SFLASH_ERR_CHIP_TIMEOUT;
}
...
uint8_t sFlash_Erase(uint32_t addr, uint32_t size)
{
	...
	{
		printf("sFlash_Erase Args Error. addr = 0x%.8X, size = 0x%.8X.\r\n", addr, size);
		return SFLASH_ERR_ARGS;
	}
...
{	
	if (addr >= SFLASH_SIZE)
	{
		printf("sFlash_WriteByte Args Error. addr = 0x%.8X.\r\n", addr);
		return SFLASH_ERR_ARGS;
	}
	...
	_UNLOCK;
	if(!is_success)
		printf("sFlash_WriteByte Failed.\r\n");
	return is_success ? SFLASH_ERR_NONE : SFLASH_ERR_CHIP_TIMEOUT;
}
...
uint8_t sFlash_WriteBytes(uint32_t addr, const void* buf, uint32_t len)
{
	...
	if( addr + len > SFLASH_SIZE )
		printf("sFlash_WriteBytes Args Error. addr = 0x%.8X, len = 0x%.8X.\r\n", addr, len);	
...
{
	if ((addr >= SFLASH_SIZE) || (p_byte == NULL))
	{
		printf("sFlash_ReadByte Args Error. addr = 0x%.8X.\r\n", addr);
		return SFLASH_ERR_ARGS;
	}
...
{	
	if ((addr + len > SFLASH_SIZE) ||
		...
		(len == 0))
	{
		printf("sFlash_ReadBytes Args Error. addr = 0x%.8X, len = 0x%.8X.\r\n", addr, len);
		return SFLASH_ERR_ARGS;
	}
...
uint8_t sFlash_BlankCheck(uint32_t addr, uint32_t size)
{
	if ((addr + size > SFLASH_SIZE) ||
		(size == 0))
	{
		printf("sFlash_ReadBytes Args Error. addr = 0x%.8X, size = 0x%.8X.\r\n", addr, size);
		return SFLASH_ERR_ARGS;
	}
